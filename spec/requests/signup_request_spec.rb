...
RSpec.describe "Signup", type: :request do
  describe "POST /signup" do
    let(:signup_endpoint) {"/v1/signup"}
    let(:user_email) {"user@tsks.mail"}
    let(:user_password) {"s"}

    context "cannot without email" do
      before :each do
        post signup_endpoint, params: {password: user_password}
      end
    ...
    context "cannot without password" do
      before :each do
        post signup_endpoint, params: {email: user_email}
      end
    ...
    context "cannot without valid email" do
      before :each do
        post signup_endpoint, params: {email: "random", password: "x"}
      end
    ...
    context "cannot already registered email" do
      ...
      before :each do
        post signup_endpoint, params: {email: registered_email, password: "s"}
      end
    ...
    context "signup succesfully" do
      ...
      before :each do
        post signup_endpoint, params: user_data
      end
